...
References: RT `CA_correct_RT` and dt `cacorrect@5.0`
...
for the then-measured clusters; 2026-07-06 re-opened the slot:
boundary-trusting reconstruction ingests clip-edge CA fringe (CLAIMS)
and the canon carries a raw-CA stage we lacked. **PORTED 2026-07-07 [SRC+EMP]:
...
`--ca-correct N`. Evidence:
`ca_correct_*.json` + `seq_spot_ca_*`; contracts
`tests/test_ca_correct.py`; flips `verify-2026-07-07/ca-flip/`.
Owner verdict 2026-07-07 (rank-1): "better in all cases" → **DEFAULT
...
Adobe on the real frame). Headroom targets: `amaze_demosaic_headroom`
scaler wrap (2026-07-07 [SRC+EMP], CLAIMS; recovery pairs with fc0).
