...
| 12 | Develop — perceptual | `develop_ops.apply_stage_12_perceptual` | PP→PP | Order **ToneCurvePV2012 → Saturation → Vibrance → Contrast → Sharpness**. Contrast is **linear-domain pivot 0.18** (not HSV); Sat/Vib are HSV; **Sharpness is a no-op stub**; tone curve here is **per-channel** (this is LR's user curve, a different op from Stage 9). | — |
