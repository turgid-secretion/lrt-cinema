...
| 12 | Develop — perceptual | `develop_ops.apply_stage_12_perceptual` | PP→PP | Order **ToneCurvePV2012 → Saturation → Vibrance → HSL → Contrast → Sharpness**. Contrast is **linear-domain pivot 0.18** (not HSV); Sat/Vib/**HSL** are HSV; **Sharpness is a no-op stub**; tone curve here is **per-channel** (this is LR's user curve, a different op from Stage 9). **HSL** = 8 hue bands × {Hue, Sat, Lum}, smooth triangular **partition-of-unity** band weights (`develop_ops.apply_hsl`, `_hsl_band_weights`); luminance is **saturation-gated** so neutrals stay neutral; identity short-circuits byte-exact. Band centres / slider magnitudes are a documented public approximation (LR's are closed). | — |
