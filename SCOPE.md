...
| LR Highlights2012/Shadows2012 → scene-referred LOCAL translation (**both intents**) | **shipped 2026-07-07 (probe-calibrated)** | `scene_tone.apply_scene_hlsh` at pipeline slot-7b (balanced scene-linear camera RGB, post scene-EV gain): fast Local-Laplacian tone core (`_fast_llf.py`, v2 2026-07-08 — detail preserved verbatim, edges remapped at the tone map's slope, absolute response at the pyramid residual's regional scale; toe-floored log channel + progressive chroma roll) + MEASURED anchor tone tables (calibrated on owner LR Classic exports of DSC_4053 at −50/−100 / +50/+100; `tools/cal_hlsh_fit.py`). Beats the best global arm at every anchor; residual per anchor in CLAIMS rows; v1 guided core superseded on owner verdict (flat look, toe-boundary artifacts). Byte-exact no-op at zero sliders; opposite slider signs mirrored-EXTRAPOLATED (no anchors yet) |
