...
| LRT's internal engine applies ≈3.1× the serialized deflicker EV (`crs:LocalExposure2012`); we apply 1× | **VERIFIED 2026-06-10 (empirically, vs LRT-internal JPGs)** | Mechanism UNKNOWN (hypothesis: LR local-mask exposure semantics ≠ naive 2^EV, or LRT-internal applies its own deflicker curve beyond the serialization). **Arbiter test pending: fresh Lightroom export** — if LR also lands ≈3×, our mask-exposure emulation has a real bug; if LR lands ≈1×, matching LRT-internal is a look-mode choice. Default stays 1.0 until arbitrated; the lever ships as `--deflicker-scale` |
